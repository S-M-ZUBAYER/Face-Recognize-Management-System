...
export const useEmployeeData = () => {
  ...
  const totalLate = useMemo(() => {
    return attendanceData.reduce((count, att) => {
      ...
      const toMinutes = (time, employee) => {
        if (!time || typeof time !== "string") {
          console.log("Invalid time for employee:", employee);
          return null;
        }
        ...
        if (isNaN(h) || isNaN(m)) {
          console.log("Invalid time format:", time, "Employee:", employee);
          return null;
        }
      ...
      // }
      const actualMin = toMinutes(actualCheckIn, employee);
      const expectedMin = toMinutes(expectedTime, employee) + latenessGraceMin;
