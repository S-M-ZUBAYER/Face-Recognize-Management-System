...
const INTERACTION_EFFECTS = {
  ...
  tap: { scale: 0.98, transition: { type: "tween", duration: 0.1 } },
};
...
// Optimized NavItem with minimal re-renders
const NavItem = memo(({ link, isActive, isPending, onNavClick }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={INTERACTION_EFFECTS.tap}
      className="relative"
    ...
    >
      ...
      >
        <AnimatePresence>
          {(isActive || isPending) && (
            <motion.div
              ...
              animate="visible"
              exit="hidden"
              className="absolute right-0 top-0 bottom-0 w-1 bg-[#004368] rounded-r"
            />
          )}
        </AnimatePresence>
        ...
        <span className="transition-colors duration-150 flex-1">
          {link.label}
        </span>
...
// Optimized UserAvatar with conditional rendering
const UserAvatar = memo(({ user, loading, error, imageUrl }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div>
        <p className="text-[#2A2A2A] text-[14px] font-[700] uppercase truncate max-w-[120px]">
          {userName}
        </p>
        {user?.adminEmail && (
          <p className="text-[#2A2A2A] text-[10px] opacity-60 truncate max-w-[120px]">
            {user.adminEmail}
          </p>
        )}
      </div>
