...
export const LateArrivalPenalty6 = () => {
  ...
  // Load existing shift penalty values from selectedRule
  useEffect(() => {
  ...
  const handleDelete = async () => {
    if (!selectedRule) {
      toast.error("No existing shift penalty settings to delete.");
      return;
    }
    setIsDeleting(true);
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className=" flex items-center w-full justify-between mt-4 gap-4">
        ...
        <button
          onClick={handleSave}
          disabled={isSaving || !dayShiftPenalty || !nightShiftPenalty}
